...
Store.where(mens_apparel: true).find_each do |store|
  puts "#{store.name} has an annual revenue of #{store.annual_revenue} and carries mens clothing"
end

Store.where("womens_apparel = ? AND annual_revenue < ?", true, 1000000).find_each do |store|
  puts "#{store.name} has an annual revenue of #{store.annual_revenue} and carries womens clothing"
end
